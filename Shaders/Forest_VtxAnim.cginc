...
//https://blogs.unity3d.com/pt/2018/06/29/book-of-the-dead-quixel-wind-scene-building-and-content-optimization-tricks/
//UnityPerMaterial
...
float3 GetBaseGustWind(float3 worldPivot, float3 worldFwd) {
    ...
    float windBaseCosInner = WIND_PI2 * (_WindTime + objectBasePhase) / _WindBaseStrengthVariancePeriod;
    float windBase = (cos(windBaseCosInner) * 0.5f + _WindBaseStrengthOffset) * _WindBaseStrength;
    float windGustCosInner = WIND_PI2 * (_WindTime + objectGustPhase) / _WindGustStrengthVariancePeriod;
...
float3 GetTreeBaseGustWind(float3 worldPivot) {
    ...
    float windBaseCosInner = WIND_PI2 * (_WindTime + objectBasePhase) / _WindTreeBaseStrengthVariancePeriod;
    float windBase = (cos(windBaseCosInner) * 0.5f + _WindTreeBaseStrengthOffset) * _WindTreeBaseStrength;
    float windGustCosInner = WIND_PI2 * (_WindTime + objectGustPhase) / _WindTreeGustStrengthVariancePeriod;
...
void AnimateVegetationHierarchyPivot(float3 worldPos, float3 worldNrm, float3 pivotData, float3 pivotColor, float3 objectRoot, out float3 worldPosOut, out float3 worldNrmOut) {
	...
	float3 lvBBaseGustWind = GetTreeBaseGustWind(objectRoot);
    ...
    if (pivotEnabled0)
    {
		...
		float3 lv0BaseGustWind = GetTreeBaseGustWind(lv0PosStatic);
        ...
        if (pivotEnabled1)
        {
			...
			float3 lv1BaseGustWind = GetTreeBaseGustWind(lv1PosStatic);
			...
			float windFlutterCos = cos(WIND_PI2 * (_WindTime + vertexFlutterPhase) / (_WindTreeFlutterGustVariancePeriod * _WindFlutterPeriodScale));
...
void AnimateVegetationSinglePivot(float3 worldPos, float3 worldNrm, float3 worldElementPivot, float3 worldElementPivotFwd, float3 pivotColor, out float3 worldPosOut, out float3 worldNrmOut) {
	...
	float3 baseGustWind = GetBaseGustWind(worldElementPivot, worldElementPivotFwd);
    ...
    float windFlutterCos = cos(WIND_PI2 * (_WindTime + vertexFlutterPhase) / (_WindFlutterGustVariancePeriod * _WindFlutterPeriodScale));
...
void AnimateVegetationSinglePivot(float3 worldPos, float3 worldNrm, float3 pivotData, float3 pivotColor, out float3 worldPosOut, out float3 worldNrmOut)
{
    uint3 packedData = asuint(pivotData);
    ...
    AnimateVegetationSinglePivot(worldPos, worldNrm, worldElementPivot, worldElementPivotFwd, pivotColor, worldPosOut, worldNrmOut);
}
...
void AnimateVegetationProceduralBranch(float3 worldPos, float3 worldNrm, float3 pivotColor, float3 objectRoot, out float3 worldPosOut, out float3 worldNrmOut) {
...
void AnimateNone(float3 worldPos, float3 worldNrm, out float3 worldPosOut, out float3 worldNrmOut)
{
	...
	#define APPLY_VEGETATION_ANIM_float(worldPos, normalWorld, pivotData, pivotColor, objectRoot, worldPosOut, worldNrmOut) { AnimateVegetationSinglePivot(worldPos, normalWorld, pivotData, pivotColor, worldPosOut, worldNrmOut); }
#elif defined(_ANIM_HIERARCHY_PIVOT)
	#define APPLY_VEGETATION_ANIM_float(worldPos, normalWorld, pivotData, pivotColor, objectRoot, worldPosOut, worldNrmOut) { AnimateVegetationHierarchyPivot(worldPos, normalWorld, pivotData, pivotColor, objectRoot, worldPosOut, worldNrmOut); }
#elif defined(_ANIM_PROCEDURAL_BRANCH)
	#define APPLY_VEGETATION_ANIM_float(worldPos, normalWorld, pivotData, pivotColor, objectRoot, worldPosOut, worldNrmOut)  { AnimateVegetationProceduralBranch(worldPos, normalWorld, pivotColor, objectRoot, worldPosOut, worldNrmOut); }
#else
	#define APPLY_VEGETATION_ANIM_float(worldPos, normalWorld, pivotData, pivotColor, objectRoot, worldPosOut, worldNrmOut) { AnimateNone(worldPos, normalWorld, worldPosOut, worldNrmOut); }
#endif

#endif //FILE_FOREST_VTXANIM
